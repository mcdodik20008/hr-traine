...
5. [Экспертная проверка заданий](#4-экспертная-проверка-заданий)
6. [Типы заданий](#5-типы-заданий)
7. [Устранение неполадок](#6-устранение-неполадок)
...
sequenceDiagram
    ...
    B-->>U: Приветствие + клавиатура /onboarding /interview /expert
    ...
    U->>TG: /expert (эксперт)
    TG->>B: Update /expert
    B->>DB: Получить задания status=checked
    B-->>EXP: Список submissions
    EXP->>B: /review <id> + оценка
    B->>DB: Обновить статус (approved/rejected)
    B-->>U: Результат проверки
```
...
:Показать клавиатуру (/onboarding, /interview, /expert, /help);
...
case (/interview)
  ...
  break
case (/expert)
  :Проверить роль EXPERT;
  if (Роль ок?) then (да)
    :Показать submissions со status=checked;
    :Команда /review <id>;
    :Получить оценку и комментарий;
    :Обновить submission (approved/rejected);
  else (нет)
...
### Use Case 1.2: Получение справки

**Актор:** Пользователь

**Основной сценарий:**
1. Пользователь отправляет команду `/help`
2. Бот выводит список доступных команд:
   ```
   Available commands:
   /start - Start the bot
   /onboarding - Start onboarding (alias /labs)
...
🤖 Бот: Available commands:
        /start - Start the bot
        /onboarding - Start onboarding (alias /labs)
        /interview - Start interview training
```

**Результат:** Пользователь видит список доступных команд
...
## 4. Экспертная проверка заданий

### Use Case 4.1: Просмотр заданий на проверку

**Актор:** Эксперт

**Предусловия:**
- Пользователь имеет роль "EXPERT" в системе
- В системе есть задания, ожидающие проверки

**Основной сценарий:**
1. Эксперт отправляет команду `/expert`
2. Бот показывает список заданий на проверку:
   ```
   Pending Submissions:
   ID: 1 | User: 5 | Step: 3
   ID: 2 | User: 7 | Step: 6
   ...
   ```
3. Эксперт выбирает задание для проверки

**Реальный пример:**
```
👤 Вы: /expert

🤖 Бот: Pending Submissions:
       ID: 1 | User: 5 | Step: 3
       ID: 2 | User: 7 | Step: 6
...
🤖 Бот: [Отправляет файл, если это FILE_UPLOAD]
       Submission 1
       Auto-Check: Basic check: {...} LLM check: {...}
       
       Please enter score (1-5) and comment (e.g., '5 Good job').

👤 Вы: 4 Хорошая работа, но можно улучшить структуру

🤖 Бот: Submission 1 graded.
```

**Альтернативный сценарий 4.1.1:** Нет заданий на проверку
```
👤 Вы: /expert

🤖 Бот: No pending submissions.
```

**Результат:** Эксперт видит список заданий, требующих проверки

---

### Use Case 4.2: Проверка задания

**Актор:** Эксперт

**Предусловия:**
- Эксперт видит список заданий через `/expert`
- Есть задания со статусом "checked" или "pending"

**Основной сценарий:**
1. Эксперт отправляет команду `/review 1` (где 1 - ID задания)
2. Бот отправляет файл задания (если это FILE_UPLOAD) или текст ответа
3. Бот показывает результаты автоматической проверки:
   - Результаты базовой валидации
   - Результаты LLM-валидации
4. Бот просит ввести оценку и комментарий: *"Please enter score (1-5) and comment (e.g., '5 Good job')."*
5. Эксперт вводит оценку и комментарий (например: "4 Хорошая работа, но можно улучшить структуру")
6. Бот сохраняет оценку:
   - Если оценка >= 3 → статус "approved"
   - Если оценка < 3 → статус "rejected"
7. Бот подтверждает: *"Submission 1 graded."*

**Альтернативный сценарий 4.2.1:** Неверный ID
- Бот отвечает: *"Submission not found."*

**Альтернативный сценарий 4.2.2:** Неверный формат оценки
- Бот отвечает: *"Invalid format. Use: <score> <comment>"*

**Альтернативный сценарий 4.2.3:** Оценка вне диапазона
- Бот отвечает: *"Score must be between 1 and 5."*

**Результат:** Задание проверено, оценено и сохранено в системе
